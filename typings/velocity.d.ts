...
// Conversion to Zepto: @tyxchen <https://github.com/tyxchen/>
// I'll PR this once I can think of a way to reference both Zepto and jQuery
...
declare namespace Velocity {
	...
	interface SequenceCall {
		e: HTMLElement|ZeptoCollection; //HTMLElement|jQuery
		p: Properties;
		o: SequenceOptions;
	}
	...
	interface VelocityStatic {
		 ...
		 */
		hook(element: HTMLElement|ZeptoCollection, cssKey: string): string;

		/**
		 * Set a hook value. Hooks are the subvalues of multi-value CSS properties.
		 * It features the same API as $.css().
		 */
		hook(element: HTMLElement|ZeptoCollection, cssKey: string, cssValue: string): void;
	}
}
